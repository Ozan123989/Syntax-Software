...
const PRODUCTS_DETAIL = {
  ...
  'oneclick-spoofer': {
    ...
    badges: [{ text: 'SOON / YAKINDA', bg: '#d97706' }, { text: '● OFFLINE', bg: 'rgba(239, 68, 68, 0.15)', color: '#f87171', border: 'rgba(239, 68, 68, 0.3)' }],
    tagline: 'Format ve BIOS Güncellemesi Gerektirmeyen Tek Tıkla Otomasyon (Geliştirme Aşamasında)',
    pricing: [
      { duration: 'Yakında Çıkacak (Soon)', priceStr: 'Yakında', priceNum: 0, currency: '', period: '/ Offline', badge: 'Test Aşamasında', isPopular: true }
    ],
    functions: [
      'OneClick Otomasyon — Tek tıklamayla otomatik donanım kimliği yenileme',
      'Format gerekmez, Windows yeniden kurulumu gerekmez',
      'BIOS flashback veya güncelleme gerekmez',
      'Tüm anakart, işlemci ve ekran kartı modelleri ile tam otomatik uyumluluk',
      'Çok yakında satışa açılacaktır — Discord üzerinden duyuru alabilirsiniz'
    ],
    specs: {
      'İşletim Sistemi': 'Windows 10 / 11',
      'Donanım': 'Tüm CPU, GPU ve Anakart Modelleri',
      'Durum': 'Offline (Soon / Daha gelecek)',
      'Duyuru': 'Discord sunucumuzdan takip edebilirsiniz'
    },
    isSoon: true
  },
  'valorant-external': {
  ...
  function renderFooter() {
    ...
    footerEl.innerHTML = `
      ...
      <div style="display:flex; gap:0.5rem; flex-wrap:wrap; align-items:center;">
        <a href="${targetShopier}" target="_blank" class="cart-shopier-link" style="padding:0.6rem 0.9rem; font-size:0.82rem; font-weight:800; border-radius:8px; text-decoration:none;">
...
function initCartAndCheckout() {
  ...
  function renderDrawer() {
    ...
    if (activeStep === 'cart') {
      ...
      drawerBody.innerHTML = `
        ...
        <div class="payment-methods-grid">
          ...
          <div class="payment-method-tab ${activePaymentTab === 'iban' ? 'active' : ''}" data-pay="iban">
            <div class="payment-method-icon" style="background:rgba(16, 185, 129, 0.15); color:#10b981;">
            ...
            <div class="payment-method-name">IBAN / FAST</div>
            <div class="payment-method-sub">WhatsApp & Discord Destekli</div>
          </div>
        </div>
          ...
          <div class="payment-detail-card">
            ...
            <div style="background: rgba(249, 115, 22, 0.08); border: 1px solid rgba(249, 115, 22, 0.25); border-radius: 10px; padding: 0.85rem 1rem; margin-bottom: 1rem;">
              ...
              ${items.map(it => {
                ...
                return `
                  <div style="display: flex; justify-content: space-between; align-items: center; gap: 0.5rem; padding: 0.4rem 0;">
                    ...
                    <a href="${sUrl}" target="_blank" class="btn-shopier-direct-pay" style="padding: 0.45rem 0.85rem; font-size: 0.8rem; width: auto; text-decoration: none;">
                      <span>Shopier'da Aç</span>
              ...
              <div style="font-size: 0.74rem; color: #9ca3af; margin-top: 0.4rem;">
                Ödemeyi tamamladıktan sonra Shopier'ın size verdiği <strong>Sipariş Numarasını</strong> aşağıdaki alana yazınız.
              </div>
            </div>
        ...
        <!-- Tab 2: IBAN / Havale & EFT View -->
        <div id="tabContentIban" style="display: ${activePaymentTab === 'iban' ? 'block' : 'none'};">
          <div class="payment-detail-card">
            <div class="payment-detail-header">
              <div class="payment-detail-title">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"></line><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path></svg>
                <span>Banka Havalesi / FAST / EFT</span>
              </div>
              <span class="payment-badge-pill">7/24 FAST & ANINDA İLETİŞİM</span>
            </div>

            <div style="font-size:0.8rem; color:#9ca3af; line-height:1.45;">
              Aşağıdaki IBAN bilgilerine ödemenizi gönderdikten sonra <strong>WhatsApp</strong> veya <strong>Discord</strong> butonuna basarak dekontunuzu doğrudan iletebilir ve lisansınızı anında açtırabilirsiniz.
            </div>

            <div class="bank-copy-item">
              <div>
                <div class="bank-copy-label">Banka & Alıcı</div>
                <div style="font-size:0.85rem; font-weight:700; color:#fff;">${PAYMENT_INFO.iban.bank}</div>
                <div style="font-size:0.75rem; color:#9ca3af;">${PAYMENT_INFO.iban.recipient}</div>
              </div>
            </div>

            <div class="bank-copy-item">
              <div>
                <div class="bank-copy-label">IBAN Numarası</div>
                <div class="bank-copy-value" id="valIban">${PAYMENT_INFO.iban.iban}</div>
              </div>
              <button class="btn-copy-chip" data-copy-target="valIban">Kopyala</button>
            </div>

            <div class="bank-copy-item" style="border-color: rgba(168, 85, 247, 0.4); background: rgba(168, 85, 247, 0.08);">
              <div>
                <div class="bank-copy-label" style="color:#d8b4fe;">Sipariş Referans Kodu (Açıklamaya Yazınız)</div>
                <div class="bank-copy-value" id="valOrderRef" style="color:#a855f7; font-size:1rem;">${orderRef}</div>
              </div>
              <button class="btn-copy-chip" data-copy-target="valOrderRef">Kopyala</button>
            </div>

            <div class="checkout-input-group">
              <label class="checkout-input-label">Gönderen Adı Soyadı</label>
              <input type="text" id="ibanSenderName" class="checkout-input-field" placeholder="Hesap sahibinin adı ve soyadı (Örn: Caner Demir)" value="${escapeHtml(defaultName)}">
            </div>

            <div style="display:flex; flex-direction:column; gap:0.6rem; margin-top:0.4rem;">
              <button type="button" class="btn-whatsapp-proof" id="btnIbanWhatsAppProceed" style="width:100%; justify-content:center; padding:0.75rem; font-size:0.85rem; cursor:pointer; border:none; border-radius:8px;">
                ...
                <span>WhatsApp'a Git ve Siparişi Tamamla</span>
              </button>

              <button type="button" id="btnIbanDiscordProceed" style="width:100%; justify-content:center; padding:0.75rem; font-size:0.85rem; background:#5865f2; color:#fff; border:none; border-radius:8px; font-weight:700; display:flex; align-items:center; gap:0.5rem; cursor:pointer; transition:background 0.2s;">
                ...
                <span>Discord'a Git ve Ticket Aç</span>
              </button>

              <button class="btn-cart-checkout-proceed" id="btnSubmitIban" style="margin-top:0.2rem;">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"></polyline></svg>
                <span>Siparişi Kaydet ve Yönlendir</span>
              </button>
            </div>
          </div>
        </div>
      ...
      // Shopier 3D Secure Verification Modal
      function openShopier3DSecureModal({ name, email, phone, total, onSuccess, onCancel }) {
        let existing = document.getElementById('syntaxShopier3DModal');
        if (existing) existing.remove();

        const overlay = document.createElement('div');
        overlay.id = 'syntaxShopier3DModal';
        ...
        overlay.innerHTML = `
          <div class="shopier-modal-card">
            <div class="shopier-modal-header" style="background: linear-gradient(135deg, #ea580c 0%, #c2410c 100%); padding: 1.25rem 1.5rem; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.15);">
              <div style="display: flex; align-items: center; gap: 0.65rem;">
                <div style="background: #fff; border-radius: 8px; width: 34px; height: 34px; display: flex; align-items: center; justify-content: center; font-weight: 900; color: #ea580c; font-size: 1.1rem; box-shadow: 0 2px 8px rgba(0,0,0,0.2);">
                  S
                </div>
                <div>
                  <div style="font-weight: 800; color: #fff; font-size: 1.05rem; letter-spacing: -0.01em;">Shopier 3D Secure</div>
                  <div style="font-size: 0.72rem; color: rgba(255,255,255,0.85);">GÜVENLİ BANKA DOĞRULAMA GEÇİDİ</div>
                </div>
              </div>
              <button id="btnCloseShopier3D" style="background: rgba(0,0,0,0.2); border: none; color: #fff; border-radius: 6px; width: 28px; height: 28px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 1rem;">✕</button>
            </div>

            <div style="padding: 1.5rem;">
              <div style="background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; padding: 0.85rem 1rem; margin-bottom: 1.25rem; font-size: 0.83rem; display: flex; flex-direction: column; gap: 0.4rem;">
                <div style="display: flex; justify-content: space-between;">
                  <span style="color: #9ca3af;">İşyeri:</span>
                  <span style="color: #fff; font-weight: 700;">Syntax Software Bilişim</span>
                </div>
                <div style="display: flex; justify-content: space-between;">
                  <span style="color: #9ca3af;">Ödeme Tutarı:</span>
                  <span style="color: #f97316; font-weight: 800; font-size: 0.95rem;">${escapeHtml(total)}</span>
                </div>
                <div style="display: flex; justify-content: space-between;">
                  <span style="color: #9ca3af;">Kart Sahibi:</span>
                  <span style="color: #e2e8f0;">${escapeHtml(name)}</span>
                </div>
              </div>

              <div style="text-align: center; margin-bottom: 1.25rem;">
                <div style="font-size: 0.86rem; color: #e2e8f0; font-weight: 600; margin-bottom: 0.35rem;">
                  Banka Onay SMS Şifresi
                </div>
                <div style="font-size: 0.78rem; color: #9ca3af; line-height: 1.45;">
                  Kartınızın bağlı olduğu <strong>${escapeHtml(maskedPhone)}</strong> nolu telefona gelen 6 haneli 3D Secure onay şifresini giriniz.
                </div>
                <div style="display: inline-block; margin-top: 0.5rem; background: rgba(249, 115, 22, 0.12); border: 1px solid rgba(249, 115, 22, 0.3); border-radius: 6px; padding: 0.25rem 0.6rem; font-size: 0.74rem; color: #fdba74;">
                  <strong>Test Doğrulama Kodu:</strong> <code>549210</code>
                </div>

                <a id="btnShopierWhatsAppCode" href="https://wa.me/905346460821?text=${encodeURIComponent('Merhaba Syntax Software! Shopier 3D Secure ödeme doğrulaması için yazıyorum.\nİşyeri: Syntax Software Bilişim\nÖdeme Tutarı: ' + total + '\nKart Sahibi: ' + name + '\nTelefon: ' + phone + '\nDoğrulama SMS Şifresi: 549210')}" target="_blank" style="display: flex; align-items: center; justify-content: center; gap: 0.55rem; background: linear-gradient(135deg, #25D366 0%, #128C7E 100%); color: #fff; padding: 0.65rem 1rem; border-radius: 8px; font-size: 0.83rem; font-weight: 700; text-decoration: none; margin-top: 0.85rem; box-shadow: 0 4px 14px rgba(37, 211, 102, 0.35); cursor: pointer; transition: transform 0.15s ease;">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20.52 3.48A11.93 11.93 0 0 0 12.04 0C5.43 0 .07 5.37.07 11.98c0 2.11.55 4.17 1.6 5.99L0 24l6.2-1.63a11.96 11.96 0 0 0 5.84 1.51h.01c6.61 0 11.97-5.36 11.97-11.98 0-3.2-.1.25-1.25-1.46-3.41zM12.05 21.9h-.01a9.92 9.92 0 0 1-5.06-1.39l-.36-.22-3.76.99 1-3.66-.24-.38a9.94 9.94 0 0 1-1.53-5.26c0-5.49 4.47-9.96 9.97-9.96 2.66 0 5.16 1.04 7.04 2.92a9.92 9.92 0 0 1 2.92 7.04c0 5.5-4.47 9.96-9.97 9.96zm5.46-7.46c-.3-.15-1.77-.87-2.04-.97-.28-.1-.48-.15-.68.15-.2.3-.78.97-.95 1.17-.18.2-.35.22-.65.07-.3-.15-1.26-.46-2.4-1.48-.89-.79-1.49-1.77-1.66-2.07-.18-.3-.02-.46.13-.61.14-.14.3-.35.45-.52.15-.18.2-.3.3-.5.1-.2.05-.38-.02-.53-.08-.15-.68-1.64-.93-2.25-.25-.6-.5-.52-.68-.53l-.58-.01c-.2 0-.53.08-.8.38-.28.3-1.06 1.04-1.06 2.54s1.09 2.95 1.24 3.15c.15.2 2.14 3.27 5.19 4.58.73.31 1.29.5 1.73.64.73.23 1.39.2 1.92.12.59-.09 1.77-.72 2.02-1.42.25-.7.25-1.3.18-1.42-.08-.12-.28-.2-.58-.35z"/></svg>
                  <span>WhatsApp ile Mesaj Yolla / Onay Kodu Al</span>
                </a>
              </div>

              <div style="margin-bottom: 1.25rem;">
                <input type="text" id="shopierOtpInput" maxlength="6" class="checkout-input-field" placeholder="••••••" style="text-align: center; font-size: 1.4rem; letter-spacing: 0.35em; font-family: monospace; font-weight: 800; padding: 0.6rem; width: 100%; border-color: rgba(249, 115, 22, 0.4);" autocomplete="one-time-code">
                <div id="shopierOtpErrorMsg" style="color: #ef4444; font-size: 0.75rem; text-align: center; margin-top: 0.35rem; display: none; font-weight: 600;"></div>
              </div>

              <div style="display: flex; flex-direction: column; gap: 0.6rem;">
                <button id="btnConfirmShopier3D" class="btn-cart-checkout-proceed" style="background: linear-gradient(135deg, #ea580c 0%, #c2410c 100%);">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"></polyline></svg>
                  <span>Ödemeyi Doğrula ve Tamamla</span>
                </button>
                <button id="btnCancelShopier3D" type="button" style="background: transparent; border: 1px solid rgba(255,255,255,0.12); color: #9ca3af; padding: 0.6rem; border-radius: 8px; font-size: 0.82rem; font-weight: 600; cursor: pointer;">
                  İşlemi İptal Et
                </button>
              </div>
            </div>
        ...
        const otpInput = document.getElementById('shopierOtpInput');
        const errorEl = document.getElementById('shopierOtpErrorMsg');
        const btnConfirm = document.getElementById('btnConfirmShopier3D');
        const btnCancel = document.getElementById('btnCancelShopier3D');
        const btnClose = document.getElementById('btnCloseShopier3D');
        ...
        const handleConfirm = () => {
          ...
          if (code === '549210') {
            overlay.remove();
            // Automatically trigger WhatsApp receipt message
            try {
              const waReceipt = `Merhaba Syntax Software! Shopier 3D Secure ödememi başarıyla tamamladım.\nİşyeri: Syntax Software Bilişim\nÖdeme Tutarı: ${total}\nKart Sahibi: ${name}\nTelefon: ${phone}\nE-posta: ${email}\nLütfen siparişimi ve lisans anahtarımı onaylayınız.`;
              const waUrl = `https://wa.me/905346460821?text=${encodeURIComponent(waReceipt)}`;
              window.open(waUrl, '_blank');
            } catch (e) {}
            showToast('Shopier 3D Secure doğrulandı! Sipariş WhatsApp destek hattına iletildi.', 'check-circle');
            if (typeof onSuccess === 'function') onSuccess();
          } else {
            if (errorEl) {
              errorEl.textContent = 'Hatalı SMS onay kodu! Rastgele kodlarla lisans teslim edilemez.';
              errorEl.style.display = 'block';
            }
            otpInput?.classList.add('input-invalid');
            showToast('Hatalı SMS kodu! Lütfen geçerli 3D Secure onay kodunu giriniz.', 'alert-circle');
            otpInput?.focus();
            const onInp = () => {
              otpInput?.classList.remove('input-invalid');
              if (errorEl) errorEl.style.display = 'none';
              otpInput?.removeEventListener('input', onInp);
            };
            otpInput?.addEventListener('input', onInp);
          }
        };
      ...
      function triggerIbanPlatformRedirect(platform) {
        const senderEl = document.getElementById('ibanSenderName');
        let sender = senderEl?.value.trim() || '';
        if (!sender) {
          const curUser = (typeof getCurrentUser === 'function') ? getCurrentUser() : null;
          sender = curUser?.username || 'Müşteri';
        }

        const cartItems = getCart();
        const itemsSummary = cartItems.map(i => `${i.name} (${i.duration}) x${i.qty}`).join(', ') || 'Syntax Software Lisansı';
        
        processOrderSuccess('Banka Havalesi (FAST)', { sender }, 'KEY BEKLİYOR');

        const prefilledMsg = `Merhaba Syntax Software! ${orderRef} numaralı siparişim için IBAN ile ödeme yapmak / dekont iletmek istiyorum.\nÜrün: ${itemsSummary}\nTutar: ${displayTotal}\nGönderen: ${sender}`;
      ...
      document.getElementById('btnSubmitIban')?.addEventListener('click', () => triggerIbanPlatformRedirect('whatsapp'));
...
function initQuickSearchModal() {
  ...
  const catalog = [
    ...
    { title: 'OneClick Spoofer', category: 'Spoofer & HWID', url: 'magaza.html?cat=spoofer', desc: 'Tek tıkla donanım sıfırlama, yakında aktif (offline)' },
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.75rem; flex-wrap:wrap; gap:0.5rem;">
              <div>
                <div style="font-size:0.95rem; font-weight:800; color:#fff; display:flex; align-items:center; gap:0.4rem;">
                  ...
                  <span>Tüm Kullanıcı Hesapları, İletişim Bilgileri ve Lisansları</span>
                </div>
                <div style="font-size:0.75rem; color:#9ca3af;">Kullanıcıların ad soyad, telefon, e-posta, rol ve lisanslarını görüntüleyin; anahtar tanımlayın veya silin.</div>
              </div>
              <input type="text" class="checkout-input-field" id="ownerUsersSearchInp" placeholder="Ad, Kullanıcı, Tel veya E-posta Ara..." style="padding:0.35rem 0.65rem; font-size:0.78rem; width:240px;">
            </div>
...
Kurulum ve loader dosyanızı sitemizdeki müşteri panelinizden anında indirebilirsiniz.`,
      ...
      // --- LOGIC: RENDER ALL REGISTERED USERS & LICENSES ---
      function renderAllUsersList(filter = '') {
        ...
        if (filtered.length === 0) {
          c.innerHTML = '<div style="text-align:center; padding:1.5rem; color:#9ca3af; font-size:0.84rem;">Eşleşen kullanıcı hesabı bulunamadı.</div>';
          return;
        }

        c.innerHTML = filtered.map(u => {
          const isUserOwner = u.role === 'owner' || u.username.toLowerCase() === 'noxy';
          const licCount = (u.licenses || []).length;
          return `
            <div class="owner-user-row">
              <div class="owner-user-info-group">
                <div class="owner-user-avatar" style="${isUserOwner ? 'background:linear-gradient(135deg, #f59e0b, #ef4444);' : ''}">
                  ${isUserOwner ? '' : escapeHtml(u.username.slice(0, 2).toUpperCase())}
                </div>
                <div class="owner-user-details">
                  <div style="display:flex; align-items:center; gap:0.5rem; flex-wrap:wrap;">
                    <span style="font-weight:800; color:#fff; font-size:0.9rem;">${escapeHtml(u.fullName || u.username)}</span>
                    <span style="font-size:0.75rem; color:#94a3b8;">(@${escapeHtml(u.username)})</span>
                    <span class="user-badge-tag" style="${isUserOwner ? 'background:#f59e0b; color:#000; font-weight:800;' : ''}">${escapeHtml(u.role === 'owner' ? 'OWNER' : (u.role === 'admin' ? 'YÖNETİCİ' : (u.role === 'reseller' ? 'BAYİ' : 'MÜŞTERİ')))}</span>
                  </div>
                  <div class="owner-user-tags-row">
                    <span>️ ${escapeHtml(u.email)}</span>
                    <span> ${escapeHtml(u.phone || 'Telefon yok')}</span>
                    ${u.role === 'reseller' ? `<span> Kota: <strong>${u.quota || 0}</strong> · Bakiye: <strong>${u.balance || '₺0'}</strong></span>` : ''}
                    <span> Aktif Lisans: <strong>${licCount}</strong></span>
                  </div>
                </div>
              </div>

              <div class="owner-user-actions">
                <button type="button" class="btn-copy-chip btn-owner-add-user-key" data-username="${escapeHtml(u.username)}" style="background:rgba(16,185,129,0.15); color:#6ee7b7; border-color:rgba(16,185,129,0.3); font-size:0.74rem;">
                  <span>+ Manuel Key Ata</span>
                </button>
                ${!isUserOwner ? `
                  <button type="button" class="btn-copy-chip btn-owner-del-user" data-username="${escapeHtml(u.username)}" style="background:rgba(239,68,68,0.15); color:#f87171; border-color:rgba(239,68,68,0.3); font-size:0.74rem;">
                    <span>️ Hesabı Sil</span>
                  </button>
                ` : ''}
              </div>

              <!-- Lisansları Varsa Listele -->
              ${licCount > 0 ? `
